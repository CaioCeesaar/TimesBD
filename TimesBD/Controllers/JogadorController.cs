...
public class JogadorController : ControllerBase
{
    ...
    public async Task<IActionResult> Get([FromQuery(Name = "name")] string? name = null, [FromHeader(Name = "Autentica")] string? autentica = null)
    {
        ...
        {
            var enderecoTime = time.EnderecoId;
            var sqlEndereco = $"SELECT * FROM Endereco WHERE Id = {enderecoTime}";
            var enderecoConsulta = await sqlConnection.QueryAsync<Endereco>(sqlEndereco);
            time.EnderecoTime = enderecoConsulta.ToList()[0];
        }
        ...
        [FromHeader(Name = "Autentica")] string? autentica = null)
    {
        ...
        {
            var endereco = await ConsultarCep(atualizaTime.EnderecoModeloTime.Cep);
            if (endereco is null)
            {
                return BadRequest("Cep inválido");
            }
    ...
    [HttpPost("Times")]
    public async Task<IActionResult> Post(TimesPost time, [FromHeader(Name = "Autentica")] string? autentica = null)
    {
        if (!ValidarAutenticacao(Request))
        {
            return BadRequest("Autenticação inválida");
    ...
    [HttpPost("Estadios")]
    public async Task<IActionResult> PostEstadios(EstadiosPost estadio, [FromHeader(Name = "Autentica")] string? autentica = null)
    {
        if (!ValidarAutenticacao(Request))
        {
            return BadRequest("Autenticação inválida");
    ...
    [HttpPatch("Estadios")]
    public async Task<IActionResult> Patch([FromQuery] int id, EstadiosPatch estadio,
        [FromHeader(Name = "Autentica")] string? autentica = null)
    {
        if (!ValidarAutenticacao(Request))
        {
        ...
        if (estadio.EnderecoModeloEstadio.Cep != null)
        {
            var endereco = await ConsultarCep(estadio.EnderecoModeloEstadio.Cep);
            if (endereco is null)
            {
                return BadRequest("Cep inválido");
            }
            ...
            sql = $"UPDATE Endereco SET Cep = '{endereco.Cep}', Logradouro = '{endereco.Logradouro}', Complemento = '{endereco.Complemento}', Bairro = '{endereco.Bairro}', Localidade = '{endereco.Localidade}', Uf = '{endereco.Uf}', Ibge = '{endereco.Ibge}', Gia = '{endereco.Gia}', Ddd = '{endereco.Ddd}', Siafi = '{endereco.Siafi}' WHERE Id = {estadio.EnderecoId}";
            await sqlConnection.ExecuteAsync(sql);
        }
        ...
        [FromHeader(Name = "Autentica")] string? autentica = null)
    {
        ...
        {
            return BadRequest("Data de jogo não pode ser maior que a data atual");
        }
